...
void ECPBuilder::build_from_json(const std::string& json_path) {
  ...
  // Collect ECP data for each atom that has it
  ...
  // Count total ECP electrons removed (for adjusting nelec)
  ...
  for (size_t u = 0; u < ecp_data.size(); u++) {
    ...
    // First shell is the "top" (local) potential
    ECPShellHandle top;
    ...
    // Remaining shells are semi-local
  ...
  for (size_t a = 0; a < mol_.natom(); a++) {
    ...
    const auto& ecp = ecp_data[u];
    size_t num_shells = ecp.shell_types.size() - 1;  // exclude top shell

    // Find shell handles for this unique ECP type
...
void ECPBuilder::build_from_file(const std::string& ecp_path) {
  ...
  num_active_ecp_ = ecp_indices_.size();
  total_ecp_electrons_ = 0;
  for (const auto& ecp : element_ecps)
    if (ecp.n_shells > 0) total_ecp_electrons_ += ecp.n_elec;
  has_ecp_ = num_active_ecp_ > 0;
  if (!has_ecp_) return;
